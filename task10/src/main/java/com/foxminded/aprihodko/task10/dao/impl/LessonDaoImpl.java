...
public class LessonDaoImpl extends AbstractCrudDao<Lesson, Long> implements LessonDao {
    ...
    public static final String FIND_BY_ID = "SELECT * FROM university.lessons WHERE lesson_id = ?";
    ...
    public static final String FIND_BY_TEACH_ID = "SELECT * FROM university.lessons WHERE teach_ref = ?";
    public static final String FIND_BY_TIME_SPAN = "SELECT * FROM university.lessons WHERE lesson_time_span = ?";
    public static final String CREATE = "INSET INTO university.lessons (lesson_id, lesson_day_of_weak, lesson_time_span, room_ref, group_ref, course_ref, teacher_ref) VALUES (?, ?, ?, ?, ?, ?, ?)";
    public static final String UPDATE = "UPDATE university.lessons SET lesson_day_of_weak =?, lesson_time_span =?, room_ref =?, group_ref =?, course_ref =?, teacher_ref =? WHERE lesson_id = ?";
    ...
    @Override
    public Optional<Lesson> findByTimeSpan(Long timeSpan) throws SQLException {
        return jdbcTemplate.query(FIND_BY_TIME_SPAN, mapper, timeSpan).stream().findFirst();
    }
